...
class CurrentLabels:
    ...
    def create_long_lant_cols(self):
        self.df['long'] = pd.to_numeric(self.df.coordinates.str.split(
            ',', expand=True).loc[:, 0].str.replace('\(', ''))
        self.df['lat'] = pd.to_numeric(self.df.coordinates.str.split(
            ',', expand=True).loc[:, 1].str.replace('\)', ''))
    ...
    def join_sector_code(self):
        ...
        def join_code_sector_inner(df):
            ...
            if state in os.listdir('data/sharp'):
                file_name = [file for file in os.listdir(
                    'data/sharp/'+state) if file.find('.shp') > 0][0]
                census_sector = gpd.read_file(
                    'data/sharp/{0:s}/{1:s}'.format(state, file_name), encoding='latin1')
                inner_df['census_code'] = inner_df['coordinate_point'].apply(
                    lambda row: census_sector.loc[census_sector.contains(row), 'CD_GEOCODI'].values).str[0]
    ...
    def save_df(self, path_to_save='data/procesada/data_with_index.pkl'):
        self.df.to_pickle(path_to_save, protocol= 4)
...
class DataWithDups:
    ...
    def remove_undefined(self):
        self.df = self.df[self.df.final_decision != 'undefined']
...
class FinalLabelsWithSector:
    ...
    def process_census_info(self, exclude_columns, cat_columns, str_columns):
        ...
        # adjust column types
        num_columns = [
            var_i for var_i in self.census.columns if var_i not in cat_columns + str_columns]
        for cat_i in cat_columns:
            self.census[cat_i] = self.census[cat_i].astype('category')

        for num_i in num_columns:
            self.census[num_i] = pd.to_numeric(
                self.census[num_i].str.replace(',', '.'), errors='coerce')
    ...
    def join_census_info(self):
        self.df = self.df.merge(self.census, left_on='census_code',
                                right_on='Cod_setor', how='left')
        self.census = None
